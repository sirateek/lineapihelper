...
class MessageApi {
   ...
   *
   * @param {string} method
   * A method to send the request to API endpoint
   * @param {string} url
   * A API Endpoint
   * @param {Object} header
   * Requets Header
  ...
  multicast(to, messages_payload, LineCredential) {
    if (!Array.isArray(to)) {
      throw new Error(
        "`to` param must be an Array only (Recieved: " + typeof replyToken + ")"
      );
    }
    if (!Array.isArray(messages_payload)) {
      throw new Error(
    ...
    var LineCredentialObj = LineCredential || this.#LineCredentalObj;
    const api_spec = this.#ApiEndpointPath.message_api_multicast();
    const api_url = this.#ApiPrefix.default_prefix() + api_spec.path;
    const request_header = LineCredentialObj.generate_request_header(
      "application/json"
    );
